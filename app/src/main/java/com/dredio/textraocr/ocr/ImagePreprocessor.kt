package com.dredio.textraocr.ocr
...
class ImagePreprocessor(
    ...
    suspend fun preprocess(
        uri: Uri,
        options: OcrPreprocessingOptions = OcrPreprocessingOptions()
        ...
        try {
            // 1) Deskew
            val deskewResult = autoDeskew(
                srcBgr = srcBgr,
                adjustmentDegrees = options.deskewAdjustmentDegrees.toDouble()
            )
            ...
            val colorSource = denoised ?: deskewed
            val gray = Mat()
            Imgproc.cvtColor(colorSource, gray, Imgproc.COLOR_BGR2GRAY)
            ...
            val outputBitmap = matToBitmap(binary ?: thresholdSource)
            ...
            PreprocessResult.Success(
                ...
                metadata = PreprocessMetadata(
                    ...
                    denoised = options.denoiseEnabled,
                    normalizedContrast = options.contrastEnabled,
                    adaptiveBinarization = options.binarizeEnabled,
                    pipeline = buildPipelineLabel(options)
                )
            )
        } catch (t: Throwable) {
            releaseAll(srcBgr)
     ...
     */
    private fun autoDeskew(srcBgr: Mat, adjustmentDegrees: Double): DeskewResult {
        val gray = Mat()
        ...
        val finalAngle = averageAngle + adjustmentDegrees
        val center = Point(srcBgr.cols() / 2.0, srcBgr.rows() / 2.0)
        val rotationMatrix = Imgproc.getRotationMatrix2D(center, finalAngle, 1.0)
        ...
        return DeskewResult(mat = rotated, angleDegrees = finalAngle)
    }

    private fun releaseAll(vararg mats: Mat?) {
        mats.forEach { mat ->
            mat?.let { runCatching { it.release() } }
        }
    }

    private fun buildPipelineLabel(options: OcrPreprocessingOptions): String {
        return buildList {
            add("deskew")
            if (options.denoiseEnabled) add("denoise")
            if (options.contrastEnabled) add("normalize")
            if (options.binarizeEnabled) add("adaptiveThreshold")
